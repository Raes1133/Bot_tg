...
def get_all_events_for_notification():
    """Получаем ВСЕ события для проверки напоминаний"""
    conn = get_db_connection()
    if not conn:
        return []
    ...
    try:
        cursor = conn.cursor()
        # Получаем все активные события (дата >= сегодня)
        ...
        logger.error(f"❌ Ошибка получения событий для напоминаний: {e}")
        if conn:
            conn.close()
        return []
...
def get_events_for_specific_time(notify_time):
    """Получаем события для конкретного времени"""
    conn = get_db_connection()
    if not conn:
        return []
